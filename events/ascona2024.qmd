...
p$what = vapply(strsplit(p$what, "talk:"), 
                \(x) paste0(x[1], if (length(x)>1) paste0("talk: <strong>", x[2], "</strong>") else ""), 
                FUN.VALUE = character(1))

# The argument 'escape=FALSE' for 'kabel' makes sure that that the HTML tags are interpreted and that, e.g., < is not converted to &lt; and > to &gt;.
# This works, but unfortunately, kableExtra::collapse_rows does not have such an argumented, and still performs exactly that conversion.
# This below a hack to revert that
k = kable(p, align = "l", escape = FALSE) |>    
      collapse_rows(columns = 1, valign = "top")
gsub("&lt;", "<", gsub("&gt;", ">", k))

# library("gt")
# k = tab_row_group(gt(p),
#    label = "day",
#    rows = everything())
```
...
<!--
```{=html}
{{< include ascona2024_schedule.html >}}
```
-->
