...
Now let's consider that the pairwise distance matrix isn't measured perfectly well, but saturates: small distances are faithful, but large distances are reported as smaller than they are. We can use the following function to model this

$$
s(x) = x \left(1-e^{-x/x_0}\right).
$$

Here, $x_0$ is the distance scale at which distances become "large". For $x\ll x_0$, $s(x)\approx x$, but for $x\gg x_0$, the value $s(x)$ is capped at $x_0$. We can plot this function, for a particular choice of $x_0$:
...
#| fig.width: 4
#| fig.height: 4
#| fig-cap:  "Heatmap representation of d2."
#| echo: !expr -1
