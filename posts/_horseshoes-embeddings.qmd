...
description: "Horseshoes and other spurious structures in low-dimensional embedding methods"
date: "2024-01-02"
categories: "Wisdom"
format: html
engine: knitr
...
webR with Quarto: <https://github.com/coatless/quarto-webr>

This post discusses how you can get spurious structures in common dimension
reduction methods. We'll see

  - how linear trajectories can emerge from "completely random" data, and
  - how horseshoe-looking lines can come from data that are really just on a straight line in the original space. 

```{webr-r}
#| label: loadpackages
set.seed(0xbedada)
pkgs = c("Rtsne", "umap", "dplyr", "ggplot2", "gridExtra", "ggfortify", "reshape2")
for (p in pkgs) {
  webr::install(p)
  require(p)
}
...
**FIXME** We see an (almost-closed) cycle. If the data were from a single-cell
...
eigD <- eigen(doubleCenter(D))
colnames(eigD$vectors) <- paste0("EV", seq_len(ncol(D)))
pairs(eigD$vectors[, 1:5])
```


**TO DO** Get rid of pals dependency

```{webr-r}
#| label: Deigenveclines1 
#| fig.width: 6
#| fig.height: 4
...
#| label: fig-dcd
plotMatrixAsImage(doubleCenter(D))
```
