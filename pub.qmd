...
format_bibentry <- function(x) {
  ...
  if (!is.null(x$citations) && !is.na(x$citations) && x$citations > 0) {
    citations <- glue::glue(
      '{ x$citations } citation(s) (source: Semantic Scholar).\n\n'
    )
  }
...
```{r, echo = FALSE}
citations <- readRDS("citation_counts.rds")
```
...
publist <- publist |>
  purrr::imap(function(x, y) {
    if (y %in% citations$BIBTEXKEY) {
      x$citations <- citations[citations$BIBTEXKEY == y, "semantic_scholar", drop = TRUE]
    } else {
      x$citations <- NA
    }
    return(x)
