...
format_bibentry <- function(x) {
  ...
  if (type == "Article" && !is.null(x$journal)) {
    ...
    journal <- paste0("*", x$institution, "*")
  } else if (!is.null(x$type)) {
    journal <- paste0("*", x$type, "*")
  ...
  # papers (with an abritrary definition of highly cited)
  if (!is.null(x$citations) && !is.na(x$citations) && x$citations > 100) {
    citations <- glue::glue(
      '{ x$citations } citations (source: OpenAlex, via the openalexR package).\n\n'
    )
  }
